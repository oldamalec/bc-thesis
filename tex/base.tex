% options:
% thesis=B bachelor's thesis
% thesis=M master's thesis
% czech thesis in Czech language
% english thesis in English language
% hidelinks remove colour boxes around hyperlinks


% arara: xelatex: { shell: yes }
% arara: makeglossaries	
% arara: biber
% arara: xelatex: { shell: yes }
% arara: xelatex: { shell: yes }
\documentclass[thesis=B,czech,hidelinks]{../template/FITthesisXE}

\usepackage{ graphicx }		% graphics files inclusion
\usepackage{ dirtree } 		% directory tree visualisation
\usepackage{ longtable } 	% tables which Pandoc use
\usepackage{ lscape }		% to be able to rotate stuff
\usepackage{ metalogo }		% for \XeLaTeX
\usepackage{ multicol }		% for multiple columns
\usepackage{ booktabs }		% for prettier tables
\usepackage{ rotating }		% for rotating pictures
\usepackage{ xcolor } 		% for custom colors
\usepackage{ mdframed }		% hack for bg colors in minted

\bibliography{sources.bib}

% make list of acronyms
\makeglossaries
\input{acronyms.tex}
\input{glossaries.tex}
\glsaddall

% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 

\acknowledgements{\input{podekovani.tex}}
\abstractCS{\input{abstrakt_cs.tex}}
\abstractEN{\input{abstrakt_en.tex}}
\input{meta.tex}

\begin{document}

%\input{hyphenation.tex}

\begin{introduction} \label{introduction}
\input{0_uvod.tex}
\end{introduction}

\input{1_dbs_portal.tex}
\input{2_metodiky_vyvoje_sw.tex}
\input{3_projektove_rizeni_dbs.tex}
\input{4_infrastruktura.tex}

\begin{conclusion} \label{conclusion}
\input{5_zaver.tex}
\end{conclusion}

\nocite{*}
\printbibliography[title={Zdroje}]

\appendix

\chapter{Seznam použitých zkratek}
\setglossarystyle{acronyms}
\printglossary[type=\acronymtype,style=acronyms]

\chapter{Slovník pojmů}
\printglossary[type=main]

\input{6_diagramy.tex}
\input{7_kviz.tex}
\input{8_hodnoceni.tex}
\input{9_medium.tex}

\end{document}
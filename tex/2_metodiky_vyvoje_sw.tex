...
\paragraph{Použití v DBS projektu}
Pro potřeby DBS projektu je Scrum také nevhodný, především jelikož předpokládá každodenní schůzky.
...
Typicky na projektu pracuje několik týmů a hlavní progragramátor pouze volí nové vlastnosti k implementaci a integruje hotové vlastnosti do výsledné aplikace.
...
V XP je \uv{předepsáno} neustále dopisovat testy pro nově vznikající komponenty. To probíhá i při vývoji DBS portálu, což je podpořeno i \emph{neustálou integrací}. Jakmile je dopsána nová funkčnost, tak při jejím \emph{push}i do \emph{Gitlab}u se automaticky spustí automatické testy.
